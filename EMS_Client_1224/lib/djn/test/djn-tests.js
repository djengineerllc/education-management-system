...
//  },
  ...
  */  
  ...
  test_handleJsonDataMethodForMultipleParams : function() {
	...
	var param2 = "param2";
	var param3 = 3;
	ServerMethodParametersReceptionTest.test_handleJsonDataMethodForMultipleParams([param, param2, param3], function(provider, response){
	  Djn.Test.checkSuccessfulResponse("test_handleJsonDataMethodForMultipleParams", response, response.result === 100);
    });
  }  
}
...
Djn.DirectStoreTest = {
  ...
  test_loadWithArguments : function() {
    var myStore = new Ext.data.DirectStore( {
      paramsAsHash:false,
      root:'items',
      paramOrder: [ 'argPassedInLoadCall', 'argPassedInBeforeLoadEvent'],
      totalProperty : 'rowCount',
...
Djn.FormSupportTest = {
  ...
  test_formPostForNonAnnotatedMethod : function() {
	var form = Djn.FormSupportTest.createForm(FormSupportTest.test_formPostForNonAnnotatedMethod);
	form.submit({
  ...
  test_handleForm : function() {
   var form = Djn.FormSupportTest.createForm(FormSupportTest.test_handleForm);
  ...
  , test_handleFormCausingServerException : function() {
	   ...
	   var form = Djn.FormSupportTest.createForm(FormSupportTest.test_handleFormCausingServerException);
	    // Test potentially "evil" characters, "=" and "&"
		...
		form.submit({
		      ...
		      failure: function(form, response) {
		    	Djn.Test.check( "test_handleFormCausingServerException", response.failureType == 'server', "Expected a server error" );
			  }
		});
  }
