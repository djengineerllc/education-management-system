...
public class BasicInfoBSImpl implements IBasicInfoBS {
	...
	public List<ProjectBO> findProjectByVO(ProjectVO projectVO)
			throws EMSException {
	...
	public List<ProfessBO> findProfessByVO(ProfessVO professVO)
			throws EMSException {
	...
	public List<EducationBO> findEducationByVO(EducationVO educationVO)
			throws EMSException {
		StringBuffer hql = new StringBuffer(
				"SELECT ebo from EducationBO ebo, UserInfoBO uibo where ebo.teacherId = uibo.id ");
		List<Object> valueParam = new ArrayList<Object>();
	...
	public List<UserInfoBO> findTeacherByEducat(EducationVO educationVO)
			throws EMSException {
		StringBuffer hql = new StringBuffer(
				"SELECT uibo from UserInfoBO uibo, EducationBO ebo where ebo.teacherId = uibo.id ");
		List<Object> valueParam = new ArrayList<Object>();
		if (educationVO.getTeacherId() != null) {
			hql.append(" and ebo.teacherId = ? ");
			valueParam.add(educationVO.getTeacherId());
	...
	public List<UserInfoVO> findUserByVO(UserInfoVO userInfoVO)
			throws EMSException {
		StringBuffer sql = new StringBuffer(
				"select a.id,a.login_name,a.user_name,a.password,a.email,a.contact,b.role_id ");
		...
		if (result != null && result.size() > 0) {
			for (int i = 0; i < result.size(); i++) {
				...
				if (obj[6] != null) {
					...
					userInfoVO_ret.setRoleName(Code.getName("Role",
							userInfoVO_ret.getRoleId() + ""));
				}
	...
	public void updateUserInfo(UserInfoVO userInfoVO)
			throws EMSRollbackableException {
		UserInfoBO userInfoBO = this.commonDAO.findById(UserInfoBO.class,
				userInfoVO.getId());
		userInfoBO.setLoginName(userInfoVO.getLoginName());
		userInfoBO.setUserName(userInfoVO.getUserName());
		...
		List<UserRoleRelBO> userRoles = this.commonDAO.find(
				UserRoleRelBO.class, new String[] { "userId" },
	...
	public void deleteUserInfo(UserInfoVO userInfoVO)
		...
		List<UserRoleRelBO> userRoleRelBO = this.commonDAO.find(
				UserRoleRelBO.class, new String[] { "userId" },
				new String[] { "=" }, new Object[] { userInfoVO.getId() });
	...
	public TermBO findCurrTerm() throws EMSException {
		return (TermBO) commonDAO.unquieResult(
				"FROM TermBO bo WHERE bo.isCurrentTerm = ?",
				Code.getValue("Indicator", "S1"));
	}

	public List<ClassBO> findClassById(List<Integer> id) throws EMSException {
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("id", id);
		return commonDAO.findListByHql("FROM ClassBO bo WHERE bo.id IN (:id)",
				paramMap);
	}
	...
	public List<CourseBO> findCourseByNo(List<String> no) throws EMSException {
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("no", no);
		return commonDAO.findListByHql(
				"FROM CourseBO bo WHERE bo.courseNo IN (:no)", paramMap);
	}

	public List<UserInfoBO> findUserInfoById(List<Integer> id)
			throws EMSException {
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("id", id);
		return commonDAO.findListByHql(
				"FROM UserInfoBO bo WHERE bo.id IN (:id)", paramMap);
	}

	public <T> List<T> getAll(Class<T> clazz, String orderBy)
			throws EMSException {
