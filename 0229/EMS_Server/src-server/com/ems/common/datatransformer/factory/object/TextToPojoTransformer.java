...
import com.ems.common.datatransformer.definition.parser.TextToPojoParserDefinition;
import com.ems.common.datatransformer.exception.GotoException;
 ...
 * @author Chiknin
 */
public class TextToPojoTransformer extends AbstractDataTransformer<String, Object> {
			...
			DataTransformerDefinition dtd, ParserDefinition pd)
	{
		...
		this.handle(textData, textData, dtd, dtd.getHandlers(), allValidateFailExceptions);
	...
	private int lineProcess(String line, TextToPojoParserDefinition pd, List<Object> textData, ValidateFailExceptions allValidateFailExceptions) {
		...
		try {
			...
			if (StringUtils.isNotBlank(gotoIndex)) {
				List<DataFieldDefinition> dfds = dataTransformerFactory.getDataFieldSet(gotoIndex);
			...
			BeanWrapper lineBean, ValidateFailExceptions validateFailExceptions) 
	{
		if (dfd instanceof TextFieldSetDefinition) {
			...
			if (DEBUG) {
				System.out.println("--->" + (StringUtils.isBlank(tfd.getDescription()) ? tfd.getName() : tfd.getDescription()) + " : |" + value + "| " + lineIndex.getLineBeginIndex() + ":" + lineIndex.getColumn());
			}
