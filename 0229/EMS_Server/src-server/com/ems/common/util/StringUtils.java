...
{ 
	...
	/**
	 * 字符串转BCD码，如webservice图片字符流转JPG文件
	 * @param asc
	 * @return
	 */
	...
	public static boolean isNullBlank(Object obj)
	{
		if (obj == null) {
			return true;
		}
	...
	*/
		 ...
		 for (int i=0;i<s.length();i++)
		 {
		     ...
		     if (c >= 0 && c <= 255) {
			    ...
			    try {
		        	...
		        	log.error(StringUtils.getExceptionStackTraceMsg(ex));
		        	b = new byte[0];
		        }
	...
	public static String getExceptionStackTraceMsg(Exception e){
		...
		for (int i = 0; i < ste.length; i++) {
			eString += "      at "+ste[i].getClassName()+"."+ste[i].getMethodName()
	...
	public static String encodeUnicode(String zhStr) { 
