package com.ems.client.action.biz.stuMag.regist;
...
public class RegistAction extends DirectAction  {
	
	private IStudentManageBS studentManageBS = this.getBean("studentManageBS", IStudentManageBS.class);
	
	@DirectMethod
	public ExtPagingVO loadRegisDisVO(JsonArray params) {
		RegistrationInfoVO registrationInfoVO = BeanUtils.toBeanFromJsonFirst(params, RegistrationInfoVO.class);
		List<RegistrationDisVO> registDisVOList = studentManageBS.findRegistDisVO(registrationInfoVO);
		return new ExtPagingVO(registDisVOList);
	}
